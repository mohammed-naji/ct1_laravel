...
<body>
    <div class="container my-5">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="{{ route('categories.create') }}" class="btn btn-dark"> <i class="fas fa-plus"></i> Add new
                Category</a>
        </div>
        <div class="mb-3">
        ...
        <table class="table table-hover table-striped">
            ...
            @foreach ($categories as $category)
                <tr>
                    <td>{{ $category->id }}</td>
                    <td><img width="100" src="{{ $category->path_image }}" alt=""></td>
                    <td>{{ $category->title }}</td>
                    <td>{{ $category->created_at->format('M d, Y') }}</td>
                    <td>{{ $category->updated_at->diffForHumans() }}</td>
                    <td>
                        <a class="btn btn-sm btn-success" href="{{ route('categories.show', $category->id) }}"><i
                                class="fas fa-eye"></i></a>
                        <a class="btn btn-sm btn-info" href="{{ route('categories.edit', $category->id) }}"><i
                                class="fas fa-edit"></i></a>
                        <form class="d-inline" action="{{ route('categorys.destroy', $category->id) }}" method="post">
                            @csrf
                            @method('delete')
